...
{
    for (auto arg : data)
        qDebug() << arg.lon_angle_deg << " " << arg.zx_angle_deg;

    auto it = std::find_if(
                data.begin(),
                data.end(),
                [](const Point &arg){ return (arg.lon_angle_deg == 0.0 && arg.zx_angle_deg == 90.0); }
    );
    if (it != data.end()) {
        qDebug() << "value found: " << (*it).lon_angle_deg << " " <<  (*it).zx_angle_deg << " " << (*it).radial;
    } else {
        //wyjatek
    }
